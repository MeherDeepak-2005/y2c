        ...
        <Center m={10}>
...

          {
            typeof window !== 'undefined' ? (
              localStorage.getItem('image') && <Button _focus={{ outline: 'none' }} onClick={() => { router.push(`/edit/vision/${info.id}`) }} role='group' variant='outline' borderBottom='.2rem solid teal' transition='all .2s' _hover={{ backgroundPosition: "100%", color: 'white' }} backgroundSize='230%' bgImage={'linear-gradient(120deg, white 0%, white 50%, teal 50%)'}>
...
export async function getStaticProps(context) {
  const getDoc = await getDocs(FireQuery(collection(db, 'visions'), where('id', '==', context.params.visionid)));
...
export async function getStaticPaths() {
  ...
  const vision = await getDocs(collection(db, 'visions'));
  const visionIds = vision.docs.map(vision => vision.data().id)

  const paths = visionIds.map((id) => ({ params: { visionid: id } }))
